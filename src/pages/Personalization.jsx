...
export default function Personalization() {
  ...
  const renderRadioGroup = (name, options) => (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
      {options.map(opt => (
        <label key={opt.value} className={`cursor-pointer px-4 py-3 rounded-xl border transition-all ${formValues[name] === opt.value ? 'bg-wellness-50 border-wellness-500 text-wellness-800 shadow-sm' : 'bg-white border-slate-200 text-slate-600 hover:border-wellness-300'}`}>
      ...
      <div className="max-w-2xl mx-auto px-4 -mt-8 relative z-20">
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <CollapsibleCard title="Cooking Situation" icon={ChefHat} isComplete={sections[1].isComplete}>
            ...
            {renderRadioGroup('cookingSituation', [
